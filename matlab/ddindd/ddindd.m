...
inj_type = 9 * ones(height(data), 1); % default: 9 %% unknown type
...
if length(injections) == length(rounds) && length(rounds) == numel(unique(data.("Inj_")))
    ...
    if all(data.Inj_ >= 1) && all(data.Inj_ <= length(rounds))
        round = rounds(data.Inj_);
        data.round = round;
    else
        error("[ERROR][round col] raw data column 'Inj_' contains undefined values.");
    end
else
...
for r = 1:n_rounds
    ...
    for s = 1:n_sensors
        ...
        strVal = D{1, s+4}; % starting value (for correction)

        y = D{:, s+4}; % filter data for sensor

        o = beast(y, season = 'none');
        cpts = o.trend.cp(~isnan(o.trend.cp)); cpts = cpts(1:3); % top 3 possible changepoints
        prob = o.trend.cpPr(~isnan(o.trend.cp)); prob = prob(1:3); % and respective probabilities

        % fitting %% find best cpt
        cpts_r2 = zeros(1, length(cpts)); cpts_opt_a = zeros(1, length(cpts)); cpts_opt_b = zeros(1, length(cpts));

        for i = 1:length(cpts) % iterate over top 3 possibile cpts
            curr_cpt = cpts(i);
            y_fit = y(curr_cpt:end); % part on which we want to fit the model function
            ...
            if length(y_fit) > 10 % minmum requirement to fit model function
                ...
                cpts_opt_a(i) = fitted_model.a; cpts_opt_b(i) = fitted_model.b; cpts_r2(i) = gof.rsquare; % save function parameters for fit w/ this cpt
            else
                disp("[INFO][func fit (round: " + r + "sensor: " + s + ")] No fitting possible. Array is to short.")
            end
        end

        cpts_length = (abs(- cpts + length(y)/3)) .^ (-.3); % inverse of distance to third
        cpts_score = cpts_r2 .* prob' .* cpts_length'; % score is product of r2, prob and cpts length 
        [~, idx] = max(cpts_score); % find the best cpt
        cpt = cpts(idx); opt_a = cpts_opt_a(idx); opt_b = cpts_opt_b(idx); r2 = cpts_r2(idx);
